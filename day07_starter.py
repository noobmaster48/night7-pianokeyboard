...
KEY_TO_FREQ = {
    ...
    pygame.K_x: 277.18,  # C#4
    pygame.K_c: 293.66,  # D4
    pygame.K_v: 311.13,  # D#4
    pygame.K_b: 329.63,  # E4
    pygame.K_n: 349.23,  # F4
    pygame.K_m: 369.99,  # F#4

    # Row 2
    pygame.K_a: 392.00,  # G4
    pygame.K_s: 415.30,  # G#4
    pygame.K_d: 440.00,  # A4
    pygame.K_f: 466.16,  # A#4
    pygame.K_g: 493.88,  # B4
    pygame.K_h: 523.25,  # C5
    pygame.K_j: 554.37,  # C#5
    pygame.K_k: 587.33,  # D5
    pygame.K_l: 622.25,  # D#5

    # Row 3
    pygame.K_q: 659.25,  # E5
    pygame.K_w: 698.46,  # F5
    pygame.K_e: 739.99,  # F#5
    pygame.K_r: 783.99,  # G5
    pygame.K_t: 830.61,  # G#5
    pygame.K_y: 880.00,  # A5
    pygame.K_u: 932.33,  # A#5
    pygame.K_i: 987.77,  # B5
    pygame.K_o: 1046.50, # C6
    pygame.K_p: 1108.73, # C#6
}
# Reverse lookup for display
FREQ_TO_NOTE = {
   277.18:"C#4", 311.13:"D#4", 369.99:"F#4", 
   415.30:"G#4", 466.16:"A#4", 554.37:"C#5", 
   622.25:"D#5", 739.99:"F#5", 830.61:"G#5", 
   932.33:"A#5", 1108.73:"C#6"
}
...
def draw_oscilloscope(surface, frequencies):
    ...
    if not frequencies:
        ...
        text = FONT.render("Use keyboard rows (zxc... iop) to play notes left → right",
                           True, TEXT_COLOR)
...
def draw_keyboard_hint(surface):
    ...
    # Bottom row labels
    bottom_keys = "ZXCVBNM"
    bottom_notes = ["C4","C#4","D4","D#4","E4","F4","F#4"]
    x_start = 60
    ...
    # Middle row labels
    middle_keys = "ASDFGHJKL"
    middle_notes = ["G4","G#4","A4","A#4","B4","C5","C#5","D5","D#5"]
    x_start2 = x_start + len(bottom_keys) * (key_size + gap) + 10

    label2 = FONT.render("MIddle:", True, TEXT_COLOR)
    ...
    # Top row labels
    top_keys = "QWERTYUIOP"
    top_notes = ["E5","F5","F#5","G5","G#5","A5","A#5","B5","C6","C#6"]
    x_start2 = x_start + len(middle_keys) * (key_size + gap) + 10

    label2 = FONT.render("Top:", True, TEXT_COLOR)
    surface.blit(label2, (x_start2 - 50, y_start + 8))

    for i, (key_char, note) in enumerate(zip(top_keys, top_notes)):
        x = x_start2 + i * (key_size + gap)
        pg_key = getattr(pygame, f"K_{key_char.lower()}")
        freq = KEY_TO_FREQ.get(pg_key)
        is_active = freq in active_frequencies if freq else False

        color = (255, 183, 77) if is_active else (40, 40, 55)
        border = (100, 100, 120)
        pygame.draw.rect(surface, color, (x, y_start, key_size, key_size), border_radius=4)
        pygame.draw.rect(surface, border, (x, y_start, key_size, key_size), 1, border_radius=4)
